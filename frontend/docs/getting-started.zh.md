...
数据库里**一个账号都没有**时,站点打开的是引导页而不是登录表单。
引导页分三个板块:

1. **代码仓库** —— 待测试的 GitLab 仓库,以及可选的 Project Access Token
   (`read_repository` 权限)。token 留空表示公开仓库。
2. **管理员账号** —— 用户名、邮箱、密码。站点的第一个账号一定是管理员。
3. **GitLab webhook** —— 无需填写:给出要粘贴到 GitLab 的 webhook URL
   (项目 → Settings → Webhooks)、要勾选的触发事件,并说明登录后到
   **Settings → Webhook** 复制 secret token。

确认后,仓库与账号在同一个事务里写入,并直接以新管理员身份登录,因此
这一页会直接把你带进 dashboard。任何一个字段被拒绝,就什么都不写。

这一页**只会出现一次**:服务器只在没有任何账号时提供它,第一个账号一旦
存在(在这里创建的、`adduser` 创建的、`seed` 创建的)就永久关闭。这也意味着
第一个账号是唯一无法用别的方式创建的账号 —— 管理员需要由管理员来创建,
所以在空库上引导页是唯一的入口。

它背后的接口是未认证的(此时还没有会话可用),所以窗口被刻意收得很窄:
**空库**在完成引导前不要暴露在公网上 —— 第一个打开引导页的人就抢到了管理员
账号。一旦存在任何账号,接口返回 `409` 且不做任何事。

## 账号

除了那第一个管理员之外,平台**没有本地注册界面**。用户通过服务器上的
`adduser` CLI 子命令创建;Web 界面负责登录,以及在管理员配置了
[GitLab 登录](#/docs/site-configuration)之后,由 GitLab 完成的自助注册 ——
这类新账号还需要管理员在 **Settings → Users** 中批准后才能登录。
...
`seed` 会创建 `demo` 账号,因此该数据库上不会再出现引导页 —— 需要管理员时用
`adduser -admin -username root -email root@example.com` 创建。
